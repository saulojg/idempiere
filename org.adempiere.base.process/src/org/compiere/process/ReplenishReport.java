...
public class ReplenishReport extends SvrProcess
{
	...
	private void fillTable (MWarehouse wh) throws Exception
	{
		...
		sql.append(" FROM M_Replenish r");
			...
			sql.append(" INNER JOIN M_Product p ON (r.M_Product_ID=p.M_Product_ID) "); // Roca
			sql.append("WHERE po.IsCurrentVendor='Y'");	//	Only Current Vendor
			sql.append(" AND r.ReplenishType<>'0'");
			sql.append(" AND po.IsActive='Y' AND r.IsActive='Y' AND p.IsBom = 'N'");
		...
		sql = new StringBuilder("UPDATE T_Replenish");
			...
			// sql.append(" THEN Level_Max - QtyOnHand + QtyReserved - QtyOrdered "); // Adempiere
			sql.append(" THEN Level_Max - QtyOnHand - QtyOrdered "); // ROCA sgo no considero lo reservado 
