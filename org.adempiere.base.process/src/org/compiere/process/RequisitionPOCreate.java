	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		if (m_order == null)
		{
			...
			if (p_LAR_Sucursal_ID != 0)
				...
				m_order.set_ValueOfColumn("LAR_Sucursal_ID", Integer.parseInt(rLine.getParent()
						.get_ValueAsString("LAR_Sucursal_ID")));
	...
	private void closeOrder() throws Exception
	{
		...
		if (m_order != null)
		{
			...
			if(AD_User_ID > 0){
				...
				statusUpdate("Req # " + reqM.getDocumentNo() + " ha sido aprobada!" + 
						"Se creo la O.C. # " + m_order.getDocumentNo() + "\nFecha=" 
						+ m_order.getDateOrdered().toLocaleString());
				
				log.info("Creo nota para el usuario " + user.getName());
				
			}
			
			message = "Se creo la Orden de Compra # <b>" + m_order.getDocumentNo() + "</b><br>";
			String result = "??";
			if(!isComplete){
