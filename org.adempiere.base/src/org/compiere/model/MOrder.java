...
public class MOrder extends X_C_Order implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// region Roca
		// msuarez 07/04/2011 no validar esto cuando cambio el warehouse desde el
		// proceso verificar almacen
		int validar = 0;
			...
			if (!newRecord && is_ValueChanged("M_Warehouse_ID")) {
				...
				for (int i = 0; i < lines.length; i++) {
					if (!lines[i].canChangeWarehouse()) {
						log.warning("MOrder. Vuelve de BeforeSave=false. Verificacion de almacenes!");
						return false;
					}
				}
			}
	...
	public String completeIt()
	{
		...
		if ( MDocType.DOCSUBTYPESO_POSOrder.equals(DocSubTypeSO)
			...
			) // ROCA comentado para que no genere Invoice si es PrepayOrder ||
			// || MDocType.DOCSUBTYPESO_PrepayOrder.equals(DocSubTypeSO)) 
		{
	...
	protected MInvoice createInvoice (MDocType dt, MInOut shipment, Timestamp invoiceDate)
	{
		...
		// dREHER
		invoice.set_Value("LAR_Sucursal_ID", (Integer) get_Value("LAR_Sucursal_ID")); // Roca
		...
		if (shipment != null)
		{
			...
			for (int i = 0; i < sLines.length; i++)
			{
				...
				int C_Tax_ID = MSysConfig.getIntValue("SinImpuesto_C_Tax_ID", 1000008, getAD_Client_ID());
	...
	protected MOrder createCounterDoc()
	{
		...
		counter.set_Value("LAR_Sucursal_ID", (Integer) this.get_Value("LAR_Sucursal_ID")); // Roca
	 ...
	 * @throws SQLException
	 */
	public BigDecimal calcOpenAmt(String trxName) {
