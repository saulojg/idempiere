...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	public BigDecimal getAllocatedAmt ()
	{
		...
		String sql = "SELECT SUM(currencyConvert(al.Amount+al.DiscountAmt+al.WriteOffAmt,"
			...
			+ " AND ah.DocStatus IN ('CO','CL') AND pay.DocStatus IN ('CO','CL')" // Roca
	...
	public boolean processIt (String processAction)
	{
		...
		return engine.processIt (processAction, docAction);
	}	//	process
	...
	public String prepareIt()
	{
		...
		if (!isSOTrx())
		{
			...
			if( C_Charge_ID != 1000065 && C_Charge_ID != 1000078 && C_Charge_ID != 1000151 ){
				...
				log.info("Es comision/sueldos, no controlo pagos a cuenta o processed para completar");
			// endregion Roca
		}
	...
	public int getC_BPartnerCliente_ID()
	{
		...
		try{
			int partnerCreditId = get_ValueAsInt("FIN_BPartnerCredit_ID");
			if(partnerCreditId > 0)
				C_BPartner_ID = DB.getSQLValue(null
						, "SELECT C_BPartner_ID FROM FIN_BPartnerCredit WHERE FIN_BPartnerCredit_ID = " + partnerCreditId);
		}
		catch(Exception ex){
			ex.printStackTrace();
		}
	...
	public void setLAR_Sucursal_ID(int id) {
		set_Value("LAR_Sucursal_ID", id);
	}
