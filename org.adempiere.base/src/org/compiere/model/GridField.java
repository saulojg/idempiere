...
//	  Do we have a mandatory rule
	...
	public String getColumnSQL(boolean withAS)
	{
		if (m_vo.ColumnSQL != null && m_vo.ColumnSQL.length() > 0)
		{
			...
			// dREHER / sg, si contiene variables, las reemplazo antes de devolver String
			if(query.indexOf("@") > -1 || query.indexOf("#") > -1){
				query = Env.parseContext(m_vo.ctx, m_vo.WindowNo, query, true, false);	
				if(query.indexOf("@") > -1 || query.indexOf("#") > -1)
					query = Env.parseContext(m_vo.ctx, m_vo.WindowNo, query, false, false);
