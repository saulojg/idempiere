...
//				}
	...
	public BigDecimal getAllocatedAmt ()
	{
		...
		// Roca if (getC_Charge_ID() != 0)
		// Roca 	return getPayAmt();
		//
	...
	public String prepareIt()
	{
		...
		{	//	see WebOrder.process
			...
			if (DOCSTATUS_WaitingPayment.equals(order.getDocStatus()))
			{
				...
				//
				// Roca German comentado para que fuincione sin generar factura ni shipment
				/*
				if (getC_Invoice_ID() == 0)
				{
				...
				if (C_Order_ID > 0) {
					...
					if (order != null) {
						if (order.getDocStatus().equals("CO") && order.isSOTrx()) { // Orden completa y de clientes
							DB.executeUpdate(
									"UPDATE C_Order SET DocStatus='WP', DocAction='CO', C_Payment_ID=null WHERE C_Order_ID="
											+ order.getC_Order_ID(),
									get_TrxName());
						}
					}
